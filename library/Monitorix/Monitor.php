 ...
 * @version    $Id: Monitor.php 59 2012-01-20 00:33:17Z markushausammann@gmail.com $
 */
 ...
 * @package    Monitorix_Monitor
 */
class Monitorix_Monitor extends Zend_Log
{
	/**
	 * 
    ...
    /**
     * Constructor takes a Zend_Log_Writer_Db instance
     *
     * @param \Zend_Log_Writer_Abstract|\Zend_Log_Writer_Db $writer
     ...
     * 
     * @param bool $toggle
     * @return Monitorix_Monitor
     */
    public function logExceptions($toggle = TRUE)
    {
        $this->loggingExceptions = $toggle;
        $this->registerControllerPlugin('Monitorix_Controller_Plugin_MonitorExceptions', $toggle);
        return $this;
    }
    ...
    /**
     * Callback function which triggers logging of the fatal error
     */
    public function registerShutdownFunction()
    {
        $error = error_get_last();
        $this->errorHandler($error['type'], $error['message'], $error['file'], $error['line'], 'Last error before shutdown. Fatal or syntax.');
    }
    ...
    public function logSlowQueries(array $adapters, $limitSeconds = 1.0, $toggle = TRUE)
    {
    	...
    	$this->registerControllerPlugin('Monitorix_Controller_Plugin_MonitorSlowQueries', $toggle);
    ...
    public function logJavascriptErrors($toggle = TRUE, $suppressJQueryCheck = FALSE)
    {
    	if ($toggle)
    	{
    		...
    		if (null === $viewRenderer->view)
			{
				...
				{
			    	throw new Monitorix_Exception('Could not init() viewRenderer.');
				}
			}
			...
			if (!$suppressJQueryCheck)
			{
				...
				if (false === $view->getPluginLoader('helper')->getPaths('ZendX_JQuery_View_Helper')) 
	    		{
	    			...
	    			{
	    				throw new Monitorix_Exception('Failed to add ZendX_JQuery_View_Helper path to view.', NULL, $exception);
	    			}
	        	}
	        	...
	        	if (!$view->jQuery()->isEnabled())
	        	{
	        		...
	        		{
	        			throw new Monitorix_Exception('Failed to enable jQuery.', NULL, $exception);
	        		}
	        	}
			}
    	}
